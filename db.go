...
func createSchema() {
	...
	-- Phase 1.2: Economy Schema (Added Oxygen)
	...
	`
	// Phase 1.3: Event Sourcing Schema
	schemaImmutable := `
	...
	`
	if _, err := db.Exec(schemaMut); err != nil {
		panic(err)
	}
	if _, err := db.Exec(schemaImmutable); err != nil {
		panic(err)
	}
